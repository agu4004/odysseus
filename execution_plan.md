...
| P1 | Trợ lý Việt hóa | 🔄 | 1/5 — phase hiện tại | 2026-06-15 |
...
| T1.4 | Persona trợ lý | System prompt tiếng Việt: giọng điệu mong muốn + **quy tắc định tuyến memory** (fact dài hạn → Second Brain sau này; ngữ cảnh phiên → `manage_memory`) + 1-2 few-shot example thao tác lịch tiếng Việt (model nhỏ hưởng lợi lớn) | Checklist chủ quan của user; prompt lưu trong Settings, export ra file backup trong repo | ⬜ |
| T1.5 | **Việt hóa intent classifier** (từ gap G-02) | `_classify_agent_request` + `_LOW_SIGNAL_RE` (`src/agent_loop.py:748`) chỉ có regex tiếng Anh → câu Việt = low_signal → agent mất tool domain. Vá: helper file mới (vd. `src/vietnamese_intent.py` chứa pattern Việt cho các domain) + vá điểm gọi **≤ 15 dòng** trong agent_loop.py (guardrail §0.3 — agent_loop là file lớn upstream). Cân nhắc mở issue/PR upstream (ảnh hưởng mọi user non-English) | Chat MỚI, agent mode: "thêm lịch ngày mai 14h: đi với lớp thạc sĩ" → log `[agent-intent]` hiện `low_signal=False, domains={notes_calendar_tasks}`, `manage_calendar` được gọi, event hiện đúng trong Calendar | ✅ |
...
|---|---|---|---|---|
| 2026-06-15 | T1.5 ✅ | `src/vietnamese_intent.py` (8 domain, 118 dòng) + patch agent_loop.py (+4 dòng) + patch tool_index.py (+12/-3 = 15 dòng) + `tests/test_vietnamese_intent.py` (18 test). pytest `not slow`: **3280 passed** (baseline 3262 + 18 mới), 0 fail. Compile clean. Log chạy thử (chờ verify thực tế trên app): câu Việt về lịch/note/web/email → đúng domain, `low_signal=False` | 18 test, diff agent_loop.py 4 dòng, diff tool_index.py 15 dòng, 0 regression | Verify hành vi thật trên app (chat mới, agent mode) cần Docker stack chạy — user tự verify theo hướng dẫn task. Lỗi giờ/ngày sai (G-03/G-06) là của T1.2, KHÔNG sửa ở T1.5 |
